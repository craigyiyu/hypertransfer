...
*最后更新：2026-08-21（周六·VIP Requests KYC 记录+转账日期排序, 分支 feat/ux-visual-polish）(后端 case 投影新增 kycRecords 聚合(sumsub_kyc_applications: status/reviewStatus/approvedAt/validUntil/submittedAt, 仅 Host/Compliance/Admin, 按通过时间倒序); 已启用 VIP 展开详情显示 KYC records 区块(通过日期+有效期); Host 视图 transfers 按 finalizedAt 倒序 + 每笔显示到账日期(无日期排最后); Python 170/170 ✅ tsc ✅ pnpm 39/39 ✅ build ✅, Playwright 实测 + 四角色 20/20 ✅, bob 已重部署。早前历史见下。）*
