...
*最后更新：2026-08-21（周六·B4+B5 无障碍与双语, 分支 feat/ux-visual-polish）(B4: Playwright+axe 双视口 20 路由审计 150→0 违规——viewport 缩放恢复、main landmark、对比度、aria-label、44px 触摸目标, 并修 CasinoOpsPortal useState TDZ 白屏 bug; B5: translations.ts 重写 en+zh 全量约 1900 行, I18nContext fallback yue→zh→en, Shell 全局挂 LanguageSwitcher, 25 页面+14 组件全部接入 useI18n, 新增 translations.test.ts 5 用例 en/zh key 对齐; tsc ✅ pnpm 39/39 ✅ build ✅, bob 已重部署, zh/en 切换实测通过, 四角色 20/20 ✅。早前历史见下。）*
