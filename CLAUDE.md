...
*最后更新：2026-08-21（周六·VIP Requests 工作台重构, 分支 feat/ux-visual-polish）(Host 工作台按反馈重构: VIP Admissions→VIP Requests, 左菜单第一项 New VIP Request; 创建表单加 VIP name(后端 patron_name 新列+幂等迁移+投影), Preferred Language 改下拉(简/繁/英/其他, 发邮件用), Intended Service→Intended deposit 金额(存 servicePurpose 如 50000 USDT), 长标签简化 Note, 移除 Route 选择(后端默认 complete_dossier); 列表改折叠式(每客户一行, 展开 timeline/KYC/意向金额/Note/逐笔转账结算), Needs your attention 可点击展开对应行; Python 168/168 ✅ tsc ✅ pnpm 39/39 ✅ build ✅, Playwright 实测 + 四角色 20/20 ✅, bob 已重部署。早前历史见下。）*
